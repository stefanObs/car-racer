#!/usr/bin/env node
/**
 * Bake Tripo Donnerbüchse hot rod into the arcade GLB (nose +Z, BodyPaint atlas).
 *
 * Source (gitignored under assets/tripo-out/donnerbuechse/): textured model.glb
 *
 * Usage: node scripts/bake-donnerbuechse-tripo.mjs
 */
...
const tripoDir = join(rootDir, "assets/tripo-out/donnerbuechse");
...
/** Map Tripo camera/face (+X) onto race forward (+Z): (x,z) → (−z, x). */
...
function halfMaxY(doc) {
  ...
  return { maxYNeg, maxYPos };
}

/** Chopped cabin + fat rear tires sit taller than the long hood — put that half at −Z. */
function cabinTowardNegZ(doc) {
  const { maxYNeg, maxYPos } = halfMaxY(doc);
...
function findSourceGlb(dir) {
  const preferred = [
    ...
    join(dir, "donnerbuechse-body.glb"),
    join(dir, "model.glb"),
  ];
...
cabinTowardNegZ(doc);
const sized = centerSitScale(doc, { targetLen: 3.8, maxSpan: 3.9 });
const orient = halfMaxY(doc);
...
console.log("donnerbuechse.glb ← Tripo hot rod", {
  src,
  bytes: bytes.byteLength,
  size: sized.size.map((v) => +v.toFixed(3)),
  cabinRear: { maxYNeg: +orient.maxYNeg.toFixed(3), maxYPos: +orient.maxYPos.toFixed(3) },
});
