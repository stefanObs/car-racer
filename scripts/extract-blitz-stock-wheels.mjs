#!/usr/bin/env node
/**
 * Split Blitz dark tire islands out of BodyPaint into recentered StockWheel_*
 * so Große Räder can upscale the same meshes without the runtime heuristic
 * carving door / rocker panels out of the body.
 *
 * Mesh space: nose +Z, width ±X (CAR_MODELS.blitz yaw 0).
 *
 *   node scripts/extract-blitz-stock-wheels.mjs
 *   node scripts/extract-blitz-stock-wheels.mjs --from=path/to/backup.glb
 */
...
const carPath = join(root, "public/models/cars/blitz.glb");
const backupPath = join(root, "assets/tripo-out/blitz/blitz-pre-wheel-split.glb");
...
function isDarkGreyPixel(r, g, b) {
  ...
  // Rubber only — keep painted arch lips / charcoal body trim on BodyPaint.
  return max < 95 && max - min < 40;
}

/** Tire volume only — keep rockers / doors / arch lips on BodyPaint. */
function inWheelVolume(x, y, z) {
  if (y < -0.01 || y > 0.42) return false;
  if (Math.abs(x) < 0.58 || Math.abs(x) > 0.92) return false;
  const front = z >= 1.05 && z <= 1.75;
  const rear = z <= -1.05 && z >= -1.75;
  return front || rear;
}
...
const bodyMesh =
  src.getRoot().listMeshes().find((m) => m.getName() === "BodyPaint") ?? src.getRoot().listMeshes()[0];
const prim = bodyMesh?.listPrimitives()[0];
if (!prim) throw new Error("Body mesh/prim missing");
...
if (!img) throw new Error("BodyPaint needs baseColor texture for dark tire mask");
...
const outBodyMesh =
  out.getRoot().listMeshes().find((m) => m.getName() === "BodyPaint") ?? out.getRoot().listMeshes()[0];
...
if (!outBodyMesh.getName()) outBodyMesh.setName("BodyPaint");
...
console.log("blitz.glb stock wheels split", {
