...
**Windows:** In PowerShell `.\start.ps1` nutzen. `start.bat` geht in cmd; aus PowerShell startet es `pwsh` oder Windows PowerShell 5.1. Portable Node bringt `npm.cmd` mit — kein vorinstalliertes npm nötig.
...
| **F6** | Mesh-Studio: Auto auf grünem Grund, Mesh-Namen + Koordinaten; **B** / **Kasten**: Rechteck ziehen, Punkte an den 12 Kanten ziehen Größe, LMB dreht das Auto, **Kasten kopieren** / C = Mesh-Raum min/max, **Zurück** / Pos1 setzt die Größe zurück. Nach dem Verschieben **Änderung kopieren** / C / RMB = Patch zum Einbacken. Rechte **Komponenten**-Liste wählt auch verdeckte Teile. **E** / **Platzieren**: Mesh versetzen, **K** eine Kante, **R** drehen, **S** 1:1 skalieren, **X** strecken |
