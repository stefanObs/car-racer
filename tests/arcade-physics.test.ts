...
describe("arcade physics — Eigenschaften scaling", () => {
  ...
  it("low Grip slides more than high Grip under hard steer", () => {
    ...
    // Match handling so Grip is the slip variable (Hot Rod also has worse Handling).
    ...
    const slipOf = (car: { vx: number; vz: number; heading: number }) => {
    ...
    expect(slipOf(hot.car)).toBeGreaterThan(slipOf(buggy.car) * 1.2);
  ...
  it("Nitro adds special acceleration; Hot Rod nitro is stronger than stock Blitz", () => {
    expect(nitroForceFor(CARS.donnerbuechse.stats.nitroBonus ?? 0, 1)).toBeGreaterThan(nitroForceFor(0, 1));

    const stock = onTrackCar(merged("blitz"), 12);
    const nitro = onTrackCar(merged("blitz"), 12);
    for (let i = 0; i < 45; i++) {
    ...
    expect(nitro.car.speed).toBeGreaterThan(stock.car.speed + 1.5);
    expect(nitro.car.nitro).toBeLessThan(1);
  });
