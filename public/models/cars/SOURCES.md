...
| **Käferkraft** (buggy) | `kaeferkraft.glb` | Tripo3D image-to-mesh (`npm run cars:bake-kaeferkraft-tripo`), then Tripo mesh segment v2 for tires + roll cage remount (`npm run cars:bake-kaeferkraft-segmented-parts`). StockCage hides under Verstärkter Rahmen; Große Räder scales `StockWheel_*` (no procedural overlays). | Generated mesh (shipped bake) |
