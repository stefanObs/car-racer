...
See [SOURCES.md](./SOURCES.md) for provenance. Rebuild from Tripo: `npm run cars:bake-bison-tripo`, `npm run cars:bake-donnerbuechse-tripo`, `npm run cars:bake-bunker-tripo` (sources under gitignored `assets/tripo-out/`). Shared spinning wheel: `../props/comic-wheel.glb` (`npm run cars:bake-wheels-tripo`).
