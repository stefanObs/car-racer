...
See [SOURCES.md](./SOURCES.md) for provenance. Rebuild from Tripo: `npm run cars:bake-bison-tripo`, `npm run cars:bake-donnerbuechse-tripo`, `npm run cars:bake-bunker-tripo` then `cars:bake-bunker-segmented-wheels` (sources under gitignored `assets/tripo-out/`). Authored tires stay in each car GLB (no shared comic-wheel overlays).
