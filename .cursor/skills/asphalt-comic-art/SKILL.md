...
HUD/menus and CSS that define the game look still follow this bible. For **3D mesh authoring** (Tripo image-to-mesh → bake GLB), also follow `.cursor/skills/tripo-3d-assets/` and regenerate `.cursor/cheatsheets/` (`npm run docs:cheatsheets`) when shipped meshes or named nodes change.
