...
|--------------|------------------------------------------|
| Full cars, FX blobs, track walls, garage props | `better_brakes` calipers; `big_wheels` procedural overlays (except Blitz / Bison / Käferkraft: Tripo-segmented `StockWheel_*` — Blitz width ×1.2, others uniform scale) |
...
**Exception — Heckspoiler:** extracted GT wing overlay (`public/models/parts/blitz-rear_spoiler.glb`). Do **not** punch the wing off the stock coupe (`npm run cars:extract-blitz-spoiler` on the car opens a hole). Wheels-only segment is `npm run cars:bake-blitz-segmented-parts`.
...
| Blitz segmented wheels | pre-split body + wheels-only segment v2 simple | `tripo-out/blitz/segment-wheels-only-v1/` | `cars:bake-blitz-segmented-parts` |
