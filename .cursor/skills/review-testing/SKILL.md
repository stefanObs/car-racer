...
- **F5 Mesh-Studio** shows only the garage car on a green void (`html.dev-mesh-inspect-mode`). Hover lists named meshes + mesh-space meters; LMB orbits the car center; RMB / **C** copies. After place-mode edits, copy is a `CRASH CIRCUIT F5 PATCH v1` block to bake forever (`npm run mesh:apply-f5-patch`). The right **Komponenten** list selects inner/occluded nodes the pick ray cannot hit. **E** / **Platzieren** turns on place mode: click a submesh, **Kante**/**K** picks one edge to drag, **Drehen**/**R** turns the whole mesh, **1:1**/**S** scales with locked ratio, **Strecken**/**X** stretches axes independently, **G** moves; no selection keeps LMB orbit. Escape / F5 exits.
