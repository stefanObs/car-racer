...
- [ ] Equipped silhouette Teile are **Tripo/extract GLBs** (not procedural boxes) — check garage with parts on; only brakes (Donnerbüchse) may be procedural; all five cars scale `StockWheel_*` for Große Räder (Blitz width-only)
